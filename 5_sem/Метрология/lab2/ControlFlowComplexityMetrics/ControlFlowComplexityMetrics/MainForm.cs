...
using ControlFlowComplexityMetrics.code_analyzer;
using ControlFlowComplexityMetrics.file_utils;
using ControlFlowComplexityMetrics.output;

namespace ControlFlowComplexityMetrics {
    public partial class MainForm : Form {
        ...
        private void AnalyzeAndSaveResults() {
            var (CL, cl, CLI, opersAmount) = _codeAnalyzer.Analyze(_programFileBuffer.Buffer!);
            var complexityMetricsConclusion = GenerateConclusion(CL, cl, CLI, opersAmount);
            SaveResultToFile(complexityMetricsConclusion);
        }

        private static string GenerateConclusion(int CL, double cl, int CLI, int opersAmount) {
            return ComplexityMetricsConclusionHtmlCreator.Create(CL, cl, CLI, opersAmount);
        }

        private void SaveResultToFile(string complexityMetricsConclusion) {
            _resultFileBuffer.Buffer = complexityMetricsConclusion;
        }
    }
}
